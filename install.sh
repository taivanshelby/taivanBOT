...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Curve-se perante ao meu criador alfa @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ESTOU CRIANDO O ARQUIVO, ENQUANTO NAO FICA PRONTO DE UMA MAMADA NO MEU CRIADOR ALFA🤯⌛',
				success: '✔️Sucesso✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo Taivan Alfa! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nSALVE!\nVocê nn está registrado no banco de dados Do Taivan Alfa tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Taivan Alfa|21`,
				}
			}
               ...
               if (data.error) return reply(data.error)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'curva-se perante ao meu criador Alfa👋', text) // ur cods
					}, 0)
                     break
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf Taivan Alfa`)
					reply(mess.wait)
