...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=+554484470869:+55 (44) 8447-0869\n' // ID do WhatsApp + nÃºmero de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SLVðŸ’ª @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}* leia as regras ou vai cinzar`
				...
				teks = `Curve-se perante ao meu criador alfa @${num.split('@')[0]}ðŸ‘‹`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'âŒ›ESTOU CRIANDO O ARQUIVO, ENQUANTO NAO FICA PRONTO DE UMA MAMADA NO MEU CRIADOR ALFAðŸ¤¯âŒ›',
				success: 'âœ”ï¸Sucessoâœ”ï¸',
				...
				only: {
					...
					ownerB: '[â—] Este comando sÃ³ pode ser usado pelo Taivan Alfa! âŒ',
					...
					Badmin: '[â—] Este comando sÃ³ pode ser usado quando o bot se torna administrador! âŒ',
                                        daftarB: `â”€â”€ ã€ŒREGISTRE-SEã€ â”€â”€\nSALVE!\nVocÃª nn estÃ¡ registrado no banco de dados Do Taivan Alfa tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Taivan Alfa|21`,
				}
			}
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '554484470869@s.whatsapp.net'
                        /******ApiKey Input******/
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite taivan|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo taivan BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, 'â•”â•â•âœªã€˜ Mencionando Todos ã€™âœªâ•â•\nâ• âž¥'+teks+'â•šâ•ã€˜ ARR157 BOT ã€™', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'curva-se perante ao meu criador AlfaðŸ‘‹', text) // ur cods
					}, 0)
                     break
               ...
               if (anu.error) return reply(anu.error)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('âŒ *NSFW NAO ATIVADO* âŒ')
                                                if (!isUser) return reply(mess.only.daftarB)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde estÃ¡ o texto? exemplo ${prefix}logowolf Taivan Alfa`)
					reply(mess.wait)
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo TAIVAN|ALFA`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}burnpaper taivanBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}lovepaper taivanBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}qowheart taivanBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}undergocean taivanBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}metalictglow taivanBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case '8bit':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}8bit taivan|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde estÃ¡ o texto hum\nExemplo: ${prefix}pubglogo taivan|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
